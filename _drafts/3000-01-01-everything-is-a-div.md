...
As I've learned about HTML I've had an odd feeling about it. There are so many tags which makes HTML feel complex and large, while at the same time lots of elements seem not that special. Also, when reading about HTML elements I see the language "*must*", "*do not*", "*indicates*", "*can only be used to represent ...*". I was curious to figure out whether this soft squishy language is just intentionally and recommendations, or how HTML actually works. The facts. No opinions or best practices.

Thus I figured out what HTML elements are the same, except for some difference in CSS. For example `<h1>` is very similar to `<h2>`, and they are the same if you change CSS `font-size` and `margin-block`. What elements actually do things?

**To cut a long story short, out of 104 elements 60 are easily represented as a `<div>`.** Many details below.
...
My rules:
- if an elements needs JavaScript to be replicated (such as `<label>`) I consider it to have special functionality
- I ignore all deprecated elements such as `<frame>`
- I ignore new not widely supported elements `<fencedframe>` and `<selectedcontent>`
- I ignore automatic ARIA roles, since the global `role` attribute can be used instead
- there are a few elements such as `<audio>` which can be replaced by `<video>` which are not in the `<div>` group. I'll be ignoring these relations. Every element not in the `<div>` group is essentially unique.
